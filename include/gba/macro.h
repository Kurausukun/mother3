...
#define H_DmaSet(dmaNum, src, dest, control)      \
{                                                 \
    vu32 *dmaRegs = (vu32 *)REG_ADDR_DMA##dmaNum; \
    *(vu16 *)REG_ADDR_DMA##dmaNum##CNT_H = 0;       \
    dmaRegs[0] = (vu32)(src);                     \
    dmaRegs[1] = (vu32)(dest);                    \
    dmaRegs[2] = (vu32)(control);                 \
}

#define H_DmaCopy(dmaNum, src, dest, size, bit)                                             \
    H_DmaSet(dmaNum,                                                                        \
           src,                                                                             \
           dest,                                                                            \
           (DMA_ENABLE | DMA_START_HBLANK | DMA_##bit##BIT | DMA_SRC_INC | DMA_DEST_RELOAD | DMA_REPEAT) << 16 \
         | ((size)/(bit/8)))

#define H_DmaCopy16(dmaNum, src, dest, size) H_DmaCopy(dmaNum, src, dest, size, 16)
#define H_DmaCopy32(dmaNum, src, dest, size) H_DmaCopy(dmaNum, src, dest, size, 32) 
